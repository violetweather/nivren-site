...
export const metadata = { title: "Nivren Studio", description: "The Nivren-first environment for understanding, building, and shipping Edition 5 software." };

export default function StudioPage() {
  return <>
    <StudioNav />
    <section className="studio-hero shell">
      <div><span className="kicker">Nivren Studio developer preview</span><h1>See what your program <em>means.</em></h1><p>A Nivren-first Code‑OSS workbench with Edition 5 intelligence, an account-free Agent surface, and the editing, terminal, Git, extension, and accessibility foundations developers already expect.</p><div className="hero-actions"><Link className="button primary" href="/studio/downloads">Download developer preview</Link><Link className="button secondary" href="/studio/docs">Explore the workflow</Link></div></div>
    ...
    <section className="proof-strip"><div className="shell proof-grid"><div><strong>Code‑OSS</strong><span>proven editor foundation</span></div><div><strong>5</strong><span>BYOK agent providers</span></div><div><strong>Edition 5</strong><span>syntax, diagnostics, and commands</span></div><div><strong>Local first</strong><span>no Nivren account required</span></div></div></section>
