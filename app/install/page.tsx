...
export default function InstallPage() {
  return <>
    ...
    <div className="shell content-shell install-content">
      ...
      <InstallChooser />
      <section className="docs-callout"><span className="kicker">Signed updates</span><h2>Choose a channel after trust is established.</h2><p>The Edition 4 beta installer supports <code>stable</code>, <code>beta</code>, and <code>nightly</code> manifests signed by a dedicated Ed25519 key. Existing verified installs can run <code>sh install.sh --channel beta --channel-key ./nivren-channel.pub</code> or <code>.\install.ps1 -Channel beta -ChannelKey .\nivren-channel.pub</code>. The installer rejects expired, tampered, or older-generation manifests and requires the archive to match both its signed digest and <code>SHA256SUMS</code>. Unix and Windows x64/ARM64 fixtures cover retained keys and generations, digest failure atomicity, rollback, and safe uninstall. Public production channel manifests, the independent signing-recovery drill, and clean-system recovery evidence remain Product Proof gates rather than promises of this beta.</p></section>
